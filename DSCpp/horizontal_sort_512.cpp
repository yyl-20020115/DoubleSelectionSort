...
__m512i HorizontalSort8(__m512i data, char* pmin, char* pmax) {
	const int stride = sizeof(data) / sizeof(*pmin);
	__m512i result = { 0 };
	int imin = HorizontalMin8(data, pmin);
	int imax = HorizontalMax8(data, pmax);
	char min = data.m512i_u8[imin];
	char max = data.m512i_u8[imax];
	if (pmin != 0)*pmin = min;
	if (pmax != 0)*pmax = max;
	result.m512i_u8[0] = min;
	result.m512i_u8[stride - 1] = max;
...
__m512i HorizontalSort16(__m512i data, short* pmin, short* pmax) {
	const int stride = sizeof(data) / sizeof(*pmin);
	__m512i result = { 0 };
	int imin = HorizontalMin16(data, pmin);
	int imax = HorizontalMax16(data, pmax);
	short min = data.m512i_u16[imin];
	short max = data.m512i_u16[imax];
	if (pmin != 0)*pmin = min;
	if (pmax != 0)*pmax = max;
	result.m512i_u16[0] = min;
	result.m512i_u16[stride - 1] = max;
...
__m512i HorizontalSort32(__m512i data, int* pmin, int* pmax) {
	const int stride = sizeof(data) / sizeof(*pmin);
	__m512i result = { 0 };
	int imin = HorizontalMin32(data, pmin);
	int imax = HorizontalMax32(data, pmax);
	int min = data.m512i_u32[imin];
	int max = data.m512i_u32[imax];
	if (pmin != 0)*pmin = min;
	if (pmax != 0)*pmax = max;
	result.m512i_u32[0] = min;
	result.m512i_u32[stride - 1] = max;
...
__m512i HorizontalSort64(__m512i data, long long* pmin, long long* pmax) {
	const int stride = sizeof(data) / sizeof(*pmin);
	__m512i result = { 0 };
	int imin = HorizontalMin64(data, pmin);
	int imax = HorizontalMax64(data, pmax);
	long long min = data.m512i_u64[imin];
	long long max = data.m512i_u64[imax];
	if (pmin != 0)*pmin = min;
	if (pmax != 0)*pmax = max;
	result.m512i_u64[0] = min;
	result.m512i_u64[stride - 1] = max;
