...
bool FastOddEvenSort256(unsigned int data[], int n)
{
	...
	for (int repeat = 0; repeat < n / 2; repeat++)
	{
		for (int part = 0; part < n; part += dbls)
		{
			...
			__m256i veven = _mm256_i32gather_epi32((int*)ptr, ipe, skip);
			__m256i vodd_ = _mm256_i32gather_epi32((int*)ptr, ipo, skip);
		...
		for (int part = 0; part < n; part += dbls)
		{
			...
			__m256i veven = _mm256_i32gather_epi32((int*)ptr, ipt, skip);
			__m256i vodd_ = _mm256_i32gather_epi32((int*)ptr, ipo, skip);
