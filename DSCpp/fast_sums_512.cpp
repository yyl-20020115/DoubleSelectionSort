...
unsigned int FastSum512(unsigned int data[], size_t size)
{
	const int stride = sizeof(__m512i) / sizeof(data[0]);
	unsigned int sum[stride] = { 0 };
	__m512i sum512 = _mm512_setzero_si512();
	__m512i load512 = _mm512_setzero_si512();
	for (size_t i = 0; i < size; i += stride)
	{
...
unsigned long long FastSum512(unsigned long long data[], size_t size)
{
	const int stride = sizeof(__m512i) / sizeof(data[0]);
	unsigned long long sum[stride >> 1] = { 0 };
	__m512i sum512 = _mm512_setzero_si512();
	__m512i load512 = _mm512_setzero_si512();
	for (size_t i = 0; i < size; i += stride)
	{
