...
void MergeData(unsigned int data[], int left, int mid, int right, unsigned int* buffer) {
	int begin1 = left;
	int begin2 = mid;
	int end1 = mid;
	int end2 = right;
...
void RecursiveMergeSort(unsigned int data[], int left, int right, unsigned int* buffer) {
	if (right - left <= 1) return;
	int mid = left + ((right - left) >> 1);
	RecursiveMergeSort(data, left, mid, buffer);
	RecursiveMergeSort(data, mid, right, buffer);
...
void RecursiveMergeSort(unsigned int a[], int n)
{
	unsigned int* buffer = new unsigned int[n];
	RecursiveMergeSort(a, 0, n, buffer);
	delete[] buffer;
}
void NonRecursiveMergeSort(unsigned int data[], int n)
{
	unsigned int* buffer = new unsigned int[n];
	int gap = 1;
	while (gap < n) {
